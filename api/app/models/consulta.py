...
class ConsultaIdentidade(Base):
    ...
    estado_id = Column(String(2), nullable=True)
    ...
    tcle_assinado = Column(Boolean, nullable=False, default=False)
    tcle_assinado_em = Column(DateTime(timezone=True), nullable=True)
    aberta_em = Column(
...
class ConsultaTcleLog(Base):
    __tablename__ = "consulta_tcle_log"
...
class ConsultaTriagem(Base):
    ...
    imc = Column(Numeric(4, 2), nullable=False)
    pa_sistolica = Column(SmallInteger, nullable=False)
    pa_diastolica = Column(SmallInteger, nullable=False)
    temperatura_c = Column(Numeric(4, 1), nullable=False)
    queixas_texto = Column(String, nullable=True)
    queixas_tags = Column(JSONB, nullable=True)
