...
def listar_para_finalizar(
    ...
    """Consultas encerradas com análise concluída aguardando revisão."""
    linhas = (
        ...
        .order_by(ConsultaIdentidade.analise_concluida_em.asc())
        .all()
    )
    return [
        ConsultaParaFinalizarItem(
            ...
            concluida_em=consulta.analise_concluida_em,
        )
...
def listar_em_processamento(
    ...
    """Consultas encerradas aguardando conclusão da análise de IA."""
    linhas = (
        db.query(ConsultaIdentidade, Paciente)
        .join(Paciente, Paciente.id == ConsultaIdentidade.paciente_id)
        .filter(
        ...
        .order_by(ConsultaIdentidade.encerrada_em.asc())
        .all()
    )
    return [
        ConsultaEmProcessamentoItem(
            ...
            ig_semanas=consulta.ig_semanas,
            ig_dias=consulta.ig_dias,
            enviada_em=consulta.encerrada_em,
        )
        for consulta, paciente in linhas
    ]
